...
namespace Orleans.Persistence.CosmosDB
{
    ...
    public class CosmosDBGrainStorage : IGrainStorage, ILifecycleParticipant<ISiloLifecycle>
    {
        ...
        public CosmosDBGrainStorage(string name, CosmosDBStorageOptions options, SerializationManager serializationManager,
            ...
            ClusterOptions clusterOptions, IGrainFactory grainFactory, ITypeResolver typeResolver, ILoggerFactory loggerFactory)
        {
            this._name = name;
            ...
            if (this._options.CustomSerializer == null && this._options.JsonSerializerSettings == null)
            {
        ...
        public async Task Init(CancellationToken ct)
        {
            ...
            try
            {
                ...
                {
                    CosmosClientOptions clientOptions = new CosmosClientOptions { ConnectionMode = this._options.ConnectionMode };
                    if (this._options.CustomSerializer != null)
                    {
                        clientOptions.Serializer = this._options.CustomSerializer;
                    }
        ...
        private string GetKeyExtensionString(GrainReference grainReference)
        {
            _ = grainReference.GetPrimaryKey(out string extension);
            return extension;
        }

        private string GetKeyString(GrainReference grainReference) => grainReference.GetPrimaryKey(out string ext).ToString(); //  $"{this._serviceId}{KeyStringSeparator}{grainReference.ToKeyString()}";
